...
frames_count = 100 #int(abs(cap.get(cv2.CAP_PROP_FRAME_COUNT)))
#print(frames_count)
...
video = cv2.VideoWriter('traffic_congestion.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), fps, (height2, width2), 1)
 
df = pd.DataFrame(index=range(0, 999999))
df.index.name = "Frames"
while True:

 
    # df = pd.DataFrame(index=range(0, frames_count))
    # df.insert( column='Frames', value=10 , loc=  )
    #df.loc[len(df)] = frames_count
   # df = df.set_index(1, inplace = True,
                           # append = True, drop = False)
    #df = df.append([""], ignore_index = True)                        
    #print(df)
    #exit()
    #df.at[framenumber, 'Frames'] = frames_count
    ret, frame = cap.read()  # import image
    ...
    if ret:  # if there is a frame continue with code
        ...
        #df = pd.DataFrame(index=range(0, frames_count))
        #df.index.name = "Frames"
        # df2 = pd.DataFrame(index=range(len(df), frames_count))
        # df2.index.name = "Frames"
        # df = df2     
        image = cv2.resize(frame, (0, 0), None, ratio, ratio)  # resize image
        ...
        lineypos = 150
        cv2.line(image, (0, lineypos), (width, lineypos), (0, 255, 0), 4)

        # line y position created to count contours
        lineypos2 = 450
        cv2.line(image, (0, lineypos2), (width, lineypos2), (0, 255, 0), 4)
        ...
        if len(cxx):  # if there are centroids in the specified area
            ...
            if not carids:  # if carids is empty
                ...
                for i in range(len(cxx)):  # loops through all centroids
                    ...
                    print("carid: " + str(carids[i]) + " added to dataframe")
                    #print(df)
                    #exit()
        ...
        cv2.rectangle(image, (0, 0), (280, 150), (0, 0, 0), -1)  # background rectangle for on-screen text

        cv2.putText(image, "Bolgedeki Araba Sayisi: " + str(currentcars), (0, 15), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0), 1)
        
        cv2.putText(image, "Toplam Arac:" + str(len(carids)), (0, 33), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0), 1)

        if flow > 4:
            cv2.putText(image, 'Trafik Yogunlugu :  Yogun Trafik ', (0, 45), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0), 1)
  

        cv2.putText(image, "Frame: " + str(framenumber) + ' of ' + str(frames_count), (0, 75), cv2.FONT_HERSHEY_SIMPLEX,
                   .5, (0, 170, 0), 1)

        cv2.putText(image, 'Zaman: ' + str(round(framenumber / fps, 2)) + ' sec of ' + str(round(frames_count / fps, 2))
